...
class Sine(PyoObject):
    ...
    def __init__(self, freq=1000, phase=0, mul=1, add=0):
        ...
        self._base_objs = [Sine_base(wrap(freq, i), wrap(
            phase, i), wrap(mul, i), wrap(add, i)) for i in range(lmax)]
        self._init_play()
    ...
    def ctrl(self, map_list=None, title=None, wxnoserver=False):
        self._map_list = [SLMapFreq(self._freq), SLMapPhase(
            self._phase), SLMapMul(self._mul)]
...
class FastSine(PyoObject):
    ...
    def __init__(self, freq=1000, initphase=0.0, quality=1, mul=1, add=0):
        ...
        self._base_objs = [
            FastSine_base(wrap(freq, i), wrap(initphase, i), wrap(
                quality, i), wrap(mul, i), wrap(add, i))
            for i in range(lmax)
        ]
        self._init_play()
    ...
    def ctrl(self, map_list=None, title=None, wxnoserver=False):
        self._map_list = [
            ...
            SLMap(0, 1, "lin", "quality", self._quality,
                  res="int", dataOnly=True),
            SLMapMul(self._mul),
        ]
        PyoObject.ctrl(self, map_list, title, wxnoserver)
...
class SineLoop(PyoObject):
    ...
    def __init__(self, freq=1000, feedback=0, mul=1, add=0):
        ...
        freq, feedback, mul, add, lmax = convertArgsToLists(
            freq, feedback, mul, add)
        self._base_objs = [
    ...
    def ctrl(self, map_list=None, title=None, wxnoserver=False):
        self._map_list = [SLMapFreq(self._freq), SLMap(
            0, 1, "lin", "feedback", self._feedback), SLMapMul(self._mul)]
        PyoObject.ctrl(self, map_list, title, wxnoserver)
...
class Phasor(PyoObject):
    ...
    def __init__(self, freq=100, phase=0, mul=1, add=0):
        ...
        self._phase = phase
        freq, phase, mul, add, lmax = convertArgsToLists(freq, phase, mul, add)
        self._base_objs = [Phasor_base(wrap(freq, i), wrap(
            phase, i), wrap(mul, i), wrap(add, i)) for i in range(lmax)]
        self._init_play()
...
class Input(PyoObject):
    ...
    def __init__(self, chnl=0, mul=1, add=0):
        ...
        self._base_objs = [Input_base(wrap(chnl, i), wrap(
            mul, i), wrap(add, i)) for i in range(lmax)]
        self._init_play()
...
class Noise(PyoObject):
    ...
    def __init__(self, mul=1, add=0):
        ...
        self._base_objs = [Noise_base(wrap(mul, i), wrap(add, i))
                           for i in range(lmax)]
        self._init_play()
...
class PinkNoise(PyoObject):
    ...
    def __init__(self, mul=1, add=0):
        ...
        PyoObject.__init__(self, mul, add)
        mul, add, lmax = convertArgsToLists(mul, add)
        self._base_objs = [PinkNoise_base(
...
class BrownNoise(PyoObject):
    ...
    def __init__(self, mul=1, add=0):
        ...
        PyoObject.__init__(self, mul, add)
        mul, add, lmax = convertArgsToLists(mul, add)
        self._base_objs = [BrownNoise_base(
            wrap(mul, i), wrap(add, i)) for i in range(lmax)]
        self._init_play()
...
class FM(PyoObject):
    ...
    def __init__(self, carrier=100, ratio=0.5, index=5, mul=1, add=0):
        ...
        self._ratio = ratio
        self._index = index
        carrier, ratio, index, mul, add, lmax = convertArgsToLists(
            carrier, ratio, index, mul, add)
        self._base_objs = [
...
class CrossFM(PyoObject):
    ...
    def __init__(self, carrier=100, ratio=0.5, ind1=2, ind2=2, mul=1, add=0):
        ...
        self._base_objs = [
            CrossFm_base(wrap(carrier, i), wrap(ratio, i), wrap(
                ind1, i), wrap(ind2, i), wrap(mul, i), wrap(add, i))
            for i in range(lmax)
        ]
        self._init_play()
...
class Blit(PyoObject):
    ...
    def __init__(self, freq=100, harms=40, mul=1, add=0):
        ...
        self._base_objs = [Blit_base(wrap(freq, i), wrap(
            harms, i), wrap(mul, i), wrap(add, i)) for i in range(lmax)]
        self._init_play()
...
class Rossler(PyoObject):
    ...
    def __init__(self, pitch=0.25, chaos=0.5, stereo=False, mul=1, add=0):
        ...
        self._chaos = chaos
        self._stereo = stereo
        pitch, chaos, mul, add, lmax = convertArgsToLists(
            pitch, chaos, mul, add)
        self._base_objs = []
        self._alt_objs = []
        for i in range(lmax):
            self._base_objs.append(Rossler_base(
                wrap(pitch, i), wrap(chaos, i), wrap(mul, i), wrap(add, i)))
            if self._stereo:
                self._base_objs.append(RosslerAlt_base(
                    self._base_objs[-1], wrap(mul, i), wrap(add, i)))
        self._init_play()
    ...
    def setPitch(self, x):
        ...
        x, lmax = convertArgsToLists(x)
        if self._stereo:
            [obj.setPitch(wrap(x, i))
             for i, obj in enumerate(self._base_objs) if (i % 2) == 0]
        else:
...
class Lorenz(PyoObject):
    ...
    def __init__(self, pitch=0.25, chaos=0.5, stereo=False, mul=1, add=0):
        ...
        self._alt_objs = []
        for i in range(lmax):
            self._base_objs.append(Lorenz_base(
                wrap(pitch, i), wrap(chaos, i), wrap(mul, i), wrap(add, i)))
            if self._stereo:
                self._base_objs.append(LorenzAlt_base(
                    self._base_objs[-1], wrap(mul, i), wrap(add, i)))
        self._init_play()
...
class ChenLee(PyoObject):
    ...
    def __init__(self, pitch=0.25, chaos=0.5, stereo=False, mul=1, add=0):
        ...
        self._alt_objs = []
        for i in range(lmax):
            self._base_objs.append(ChenLee_base(
                wrap(pitch, i), wrap(chaos, i), wrap(mul, i), wrap(add, i)))
            if self._stereo:
                self._base_objs.append(ChenLeeAlt_base(
                    self._base_objs[-1], wrap(mul, i), wrap(add, i)))
        self._init_play()
...
class LFO(PyoObject):
    ...
    def __init__(self, freq=100, sharp=0.5, type=0, mul=1, add=0):
        ...
        self._sharp = sharp
        self._type = type
        freq, sharp, type, mul, add, lmax = convertArgsToLists(
            freq, sharp, type, mul, add)
        self._base_objs = [
    ...
    def ctrl(self, map_list=None, title=None, wxnoserver=False):
        self._map_list = [
            SLMap(0.1, self.getSamplingRate() *
                  0.25, "log", "freq", self._freq),
...
class SumOsc(PyoObject):
    ...
    def __init__(self, freq=100, ratio=0.5, index=0.5, mul=1, add=0):
        ...
        self._ratio = ratio
        self._index = index
        freq, ratio, index, mul, add, lmax = convertArgsToLists(
            freq, ratio, index, mul, add)
        self._base_objs = [
...
class SuperSaw(PyoObject):
    ...
    def __init__(self, freq=100, detune=0.5, bal=0.7, mul=1, add=0):
        ...
        self._detune = detune
        self._bal = bal
        freq, detune, bal, mul, add, lmax = convertArgsToLists(
            freq, detune, bal, mul, add)
        self._base_objs = [
...
class RCOsc(PyoObject):
    ...
    def __init__(self, freq=100, sharp=0.25, mul=1, add=0):
        ...
        self._base_objs = [RCOsc_base(wrap(freq, i), wrap(
            sharp, i), wrap(mul, i), wrap(add, i)) for i in range(lmax)]
        self._init_play()
    ...
    def ctrl(self, map_list=None, title=None, wxnoserver=False):
        self._map_list = [SLMapFreq(self._freq), SLMap(
            0, 1, "lin", "sharp", self._sharp), SLMapMul(self._mul)]
        PyoObject.ctrl(self, map_list, title, wxnoserver)
